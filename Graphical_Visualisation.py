...
def Emotion_Analysis(img):
    ...
    for (x, y, w, h) in faces:
        ...
        plt.bar(EMOTIONS, data, color=['red','blue','yellow','green','orange','brown','violet'],
                width=0.4)
...
def Text_Analysis(text):
    text1 = text
    li=te.get_emotion(text)
    emo=max(li, key=li.get)
    emotion = list(li.keys())
    values = list(li.values())
    fig = plt.figure(figsize=(8, 5))

    # Creating the bar plot
    plt.bar(emotion, values, color=['green','red','orange','yellow','violet'],
            width=0.4)

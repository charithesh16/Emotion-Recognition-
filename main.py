...
from Graphical_Visualisation import Emotion_Analysis, Text_Analysis
...
def textanalysis():
    ...
    sentence = mood(result[1])
    activity = activities(result[1])
    link = provide_url(result[1])
    return render_template('textan.html',orig=result[1], bar=result[0]+'.png', music=result[1],
                           sentence=sentence, activity=activity, image=result[1], link=link)
